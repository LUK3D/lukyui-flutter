//The base for this implementation is from https://github.com/marcojakob/dart-event-bus
import 'dart:async';

/// ## LukyEventSystem
/// Dispatches events to listeners using the Dart [Stream] API. The [LukyEventSystem]
...
///
/// Not all events should be broadcasted through the [LukyEventSystem] but only those of
/// general interest.
///
/// Events are normal Dart objects. By specifying a class, listeners can
/// filter events.
///
class LukyEventSystem {
  final StreamController _streamController;

  /// Controller for the event system stream.
  StreamController get streamController => _streamController;

  /// Creates an [LukyEventSystem].
  ///
  ...
  /// completed.
  LukyEventSystem({bool sync = false})
  ...
  LukyEventSystem.customController(StreamController controller)
  ...
  /// ## emit
  /// Fires a new event on the event system with the specified [event].
  ///
  void emit(event) {
  ...
  /// ## destroy
  /// Destroy this [LukyEventSystem]. This is generally only in a testing context.
  ///
  void destroy() {
